...
import { databaseIsReachable, withRealDatabase } from './database'
...
/*
 * A real connection, or a loud skip.
 *
 * The paths below that write an upload read its authorization from the
 * database, so they need one — and being the only tests in the repository that
...
const itWithDatabase = HAS_DATABASE
  ? (name: string, body: (db: Database) => Promise<void>) =>
      it(name, () => withRealDatabase(body))
  : (name: string, _body: (db: Database) => Promise<void>) =>
      it.skip(`${name} (needs a Postgres at the Hyperdrive binding)`, () => undefined)
...
describe('storage on a machine with no bucket', () => {
  ...
  /*
   * A database that refuses to be used.
   *
   * Most paths here never reach one — they are 403s, 404s and byte round trips
   * through the bucket — and a stand-in that throws says so. It also names the
   * remedy, so a test that starts reading rows fails with the reason rather than
   * a confusing empty result.
   *
   * The four that *do* need one take `realDatabase` below instead.
   */
  const noDatabase = new Proxy({} as Database, {
    get() {
      throw new Error(
        'The runtime suite has no database. Use withRealDatabase, or move this '
        + 'test to test/service/.',
      )
    },
  })
  const context = (extra: Partial<AppBindings> = {}, db: Database = noDatabase) =>
    ({ db, env: bindings(extra) })
  ...
  itWithDatabase('refuses an upload id that was never issued', async (db) => {
  ...
  describe('the preflight the browser sends first', () => {
    ...
    itWithDatabase('authorizes the upload the authorization asked the caller to make', async (db) => {
    ...
    itWithDatabase('answers the request itself with the headers too', async () => {
  ...
  itWithDatabase('serves a stored object as an attachment, and never inline', async () => {
  ...
  itWithDatabase('measures the body even when nothing declared its length', async (db) => {
    /*
  ...
  itWithDatabase('refuses an upload address with no id in it', async (db) => {
...
describe('storage in Cloudinary', () => {
  ...
  it('refuses an unrecognised provider rather than picking one', () => {
    expect(() => backend({ STORAGE_TRANSPORT: 'gcs' as never })).toThrow(
