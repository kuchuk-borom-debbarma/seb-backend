 ...
 */
import { SELF, env } from '../support/worker'
...
/*
 * Deliberately looser than `Partial<AppBindings>`. The whole point of these
 * tests is what the factory does with a value it should refuse — an unset
 * environment, a padded one, a key that is not there — and a type that only
 * admits the valid values cannot express the question.
 */
const bindings = (extra: Record<string, unknown> = {}) => extra as unknown as AppBindings
...
describe('choosing a transport', () => {
  ...
  it('delivers for real once the environment says it is deployed', () => {
    ...
    expect(notificationTransport(bindings({ ...env, ENVIRONMENT: ' Develop ' })).name)
      .toBe('pingram')
  })
...
describe('the provider adapter', () => {
  ...
  it('still refuses with the status alone when an attachment is rejected', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response('attachment too large: application-SEP-2026-ABCDEFGH.pdf', { status: 413 }),
    )
    const thrown = await transport.send({
      ...message,
      attachments: [{
        filename: 'application-SEP-2026-ABCDEFGH.pdf',
        contentType: 'application/pdf',
    ...
    expect(thrown).not.toContain('application-SEP-2026-ABCDEFGH.pdf')
  })
