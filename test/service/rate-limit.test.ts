 ...
 * padded one, a flag that must never be honoured in production — and a type
 * narrowed to the configured literal cannot express the question.
 */
  ...
  it('limits every named operation in a document, not just the first', async () => {
    ...
    const both = () => post(/* GraphQL */ `mutation {
      seb { application { issueDocumentUpload(input: {
        ...
        fieldKey: "DPR", expectedDocumentVersion: 0,
