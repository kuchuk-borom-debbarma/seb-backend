...
describe('the audit history', () => {
  ...
  it('seeks the created index instead of scanning, with no filters at all', async () => {
    ...
    /*
     * "SCAN ... USING COVERING INDEX" is the good outcome, not a warning:
     * SQLite walks the index in order and never visits the table, which is
     * exactly what a keyset page wants. The bad outcome is a bare
     * "SCAN core_audit_event" with no index named after it.
     */
    expect(detail).toContain('core_audit_event_created_idx')
    expect(detail).not.toMatch(/SCAN core_audit_event(?! USING)/u)
    // And no sort at all: a temp b-tree is what this index exists to avoid.
